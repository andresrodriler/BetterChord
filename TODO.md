...
- Deploy: frontend on Vercel, backend on Render via Docker. **Plumbing done + container-verified, Phase 7** -- `Dockerfile` + `docker/fetch_hf_data.py` + `.dockerignore` + `DEPLOYMENT.md`; full local build, container run, and all 4 endpoints (`/chords`, `/voicings/C`, `/songs/C`, `/identify`) hit for real. **ONNX follow-up (next session):** swapped deployed CNN inference from PyTorch to ONNX Runtime (training untouched, additive export via `betterchord/training_scripts/export_onnx.py`) purely for memory/cost -- the person can't afford Render's 2 GB Standard tier. Real before/after: baseline anon ~345 MiB -> ~48 MiB; after first `/identify` anon ~630 MiB -> ~390-408 MiB; peak under load ~790 MiB -> ~570 MiB. **Verified it fits a hard 512 MB limit with no OOM-kill** (all 4 endpoints 200, `Cm9` identified) -- confirmed twice across two sessions. A tuning follow-up (drop matplotlib/pandas: image 2.32->2.09 GB; onnxruntime arena-off + single-thread: anon headroom ~104->~125 MiB) helped a little but it still runs pinned to the 512 ceiling -- **fits, not comfortably; move to 1 GB if it misbehaves in prod.** Cold first request ~17s (numba JIT), warm ~0.19s unconstrained / ~0.45s under the 512 limit; ONNX latency ~= torch's recorded numbers. torch vs ONNX numerically equivalent (per-logit max|Δ| ~1e-6; both -> `Cm9 @ 0.214769`). **Manual step still owed: upload a fresh `chord_cnn.onnx` to the private HF repo after every retrain** (workflow in DEPLOYMENT.md). Creating the Render/Vercel services in their dashboards is still the person's step (follow `DEPLOYMENT.md`).
