...
- ~~API_URL is currently hardcoded to 127.0.0.1:8000~~ -- **done, Phase 7.** Now `import.meta.env.VITE_API_URL` in `frontend/src/lib/api.js` (the single source; App.jsx doesn't call the API directly), local-dev fallback kept. `frontend/.env.example` documents it; set the real value in Vercel's dashboard. See `DEPLOYMENT.md` + CLAUDE.md's Phase 7 entry.
  - ~~Real-device mobile test still owed~~ -- **done, Phase 9.** The live deploy went through multiple real-phone (real iOS Safari + real mobile Chrome, not emulation) fix rounds: Vercel SPA-routing 404 fix, iOS "Take Video" file-picker (accepted platform limitation after two suppression attempts), ambient-diagram text overlap, How It Works CNN diagram + full accordion redesign, Results density pass + sticky quick-jump nav, Chord Overview spacing, loading-dots centering (a tip-text/arrow layout issue, not the dots), and the video-container waveform-analysis investigation. See CLAUDE.md's Phase 9 entry.
- Deploy backend. **Plumbing done + solid, Phase 7** -- `Dockerfile` + `docker/fetch_hf_data.py` + `.dockerignore` + `requirements-deploy.txt` + `DEPLOYMENT.md`; CNN inference converted PyTorch -> ONNX Runtime for memory (training untouched, additive export via `betterchord/training_scripts/export_onnx.py`), verified numerically equivalent on 13,380 real audio files (0 mismatches). **Manual step owed for any future retrain: re-run `export_onnx.py` and upload the fresh `chord_cnn.onnx` to the private HF repo** (workflow in DEPLOYMENT.md).
  - ~~Get it running on Render's free 512 MB tier~~ -- **done, Phase 9.** Phase 7 had concluded this was abandoned (the ONNX image passed a local `docker run --memory=512m` test but the real Render free instance OOM-killed on cold-start `/identify` -- Events tab `exit 137` -- because numba JIT-compiled librosa's CQT kernels live on that first request). **Phase 9 fixed the actual cause:** `docker/warmup_numba.py` bakes the compiled kernels into an image layer (`NUMBA_CPU_NAME=generic` keeps the cache portable across the build/run CPU; a BuildKit sub-second-mtime-truncation bug that was silently invalidating the cache was found and fixed). Cold start dropped ~17 s / 512-MiB-pinned / 644-834 reclaim events -> **~3 s / ~448 MiB / 0 reclaim events / 0 OOM kills**, confirmed on the real Render deploy (Events tab + timed cold `/identify` requests), chord output bit-identical across a 50-file sweep. **Render free is the live backend.** The earlier thread-cap `ENV` + per-request memory release are still in place. Full story: CLAUDE.md's Phase 9 entry.
- ~~Phase 9 -- deploy the backend on Google Cloud Run~~ -- **NOT pursued; superseded, Phase 9.** Cloud Run was only ever the plan *because* Render free couldn't survive the cold-start OOM. The numba pre-cache fix (bullet above) removed that OOM, so there's no reason to take on Cloud Run's extra setup (Secret Manager, `cloudbuild.yaml`, IAM/service-account, an explicit `--concurrency=1`/`2` to avoid re-creating the same OOM). **Decision: stay on Render's free tier.** If it ever misbehaves under real load, Render's 1 GB paid tier is the next step, not a platform migration. Both Vercel and Render auto-deploy from the `Anders` branch (confirmed in each dashboard); `main` is stale and deploys nowhere.
- ~~Whatever backend host needs ffmpeg available~~ -- **done, Phase 7.** The `Dockerfile` installs ffmpeg via `apt-get` (needed only for the browser WebM/Opus record path; MP3/common uploads decode natively via libsndfile).
...
- **Phase 8 also covers, added at Phase 9 close-out (now that the site is
  live):** analytics (Vercel Analytics is the natural first fit given the
  host, but not the only option -- investigate/decide when the phase
  starts, then document the choice); a custom domain (option exists, set
  aside earlier over cost/card -- revisit as a real decision, it changes
  `ALLOWED_ORIGINS` / `VITE_API_URL`); and an open-ended pass on other
  live-site basics (error monitoring, uptime/health checks, etc. -- a
  discussion, not a predetermined list). Full framing in CLAUDE.md's
  Phase 8 entry.
