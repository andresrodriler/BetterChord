...
- ~~API_URL is currently hardcoded to 127.0.0.1:8000~~ -- **done, Phase 7.** Now `import.meta.env.VITE_API_URL` in `frontend/src/lib/api.js` (the single source; App.jsx doesn't call the API directly), local-dev fallback kept. `frontend/.env.example` documents it; set the real value in Vercel's dashboard. See `DEPLOYMENT.md` + CLAUDE.md's Phase 7 entry.
  - Real-device mobile test is now UNBLOCKED but still owed: do it once a real deploy is live (open the Vercel URL in actual mobile Chrome/Safari). Phase 6 was Playwright device-emulation only.
- Deploy backend. **Plumbing done + solid, Phase 7** -- `Dockerfile` + `docker/fetch_hf_data.py` + `.dockerignore` + `requirements-deploy.txt` + `DEPLOYMENT.md`; CNN inference converted PyTorch -> ONNX Runtime for memory (training untouched, additive export via `betterchord/training_scripts/export_onnx.py`), verified numerically equivalent on 13,380 real audio files (0 mismatches). **Manual step owed for any future retrain: re-run `export_onnx.py` and upload the fresh `chord_cnn.onnx` to the private HF repo** (workflow in DEPLOYMENT.md).
  - ~~Get it running on Render's free 512 MB tier~~ -- **concluded / abandoned, Phase 7.** The ONNX image passed a local `docker run --memory=512m` test but the real deployed Render free instance OOM-killed on cold-start `/identify` (Events tab `exit 137`); a bounded thread-cap + `gc.collect()`/`malloc_trim` squeeze made the warm path reliable but not the cold-start transient (1 of 2 cold starts still OOM'd; Render free spins down after ~15 min idle so every first visit hits it). Platform-fit conclusion, not code-quality -- the backend is solid. Full arc + real numbers in CLAUDE.md's Phase 7 "Final Render addendum". Render's 2 GB Standard tier is off the table (cost). The squeeze-attempt changes (thread-cap `ENV` + per-request memory release) are kept, not reverted -- good practice on any host, and a smaller RSS directly cuts cost on a memory-time-billed platform.
- **Phase 9 -- deploy the backend on Google Cloud Run.** (Phase 8 is already Publishing/release decisions.) Not started. Bigger lift than Render: needs Secret Manager (HF token + dataset repo id, referenced from the build, not baked into the image), a `cloudbuild.yaml` (or `gcloud run deploy --source` with the repo-root Dockerfile), IAM/service-account setup, and -- important -- an **explicit low concurrency setting** (`--concurrency=1` or `2`): Cloud Run defaults to 80 concurrent requests per instance, which would stack many simultaneous `/identify` calls (each ~400 MB RSS + the cold numba JIT) into one container and reproduce the exact OOM this phase fought. Also pick a memory size deliberately (512 MiB is proven too tight; 1 GiB is the sensible floor). Vercel frontend already points at a configurable `VITE_API_URL`, so only that env var changes once the Cloud Run URL exists. (The still-owed real-device mobile test -- see the API_URL bullet above -- happens once this deploy is live.)
